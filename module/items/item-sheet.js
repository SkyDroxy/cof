...
export class CofItemSheet extends ItemSheet {
    ...
    html.find(".capacity-activated-toggle").click((ev) => {
      ...
      // Capacité non rattachée à un acteur
        ...
        if (data.effects.length > 0) {
          data.effects.forEach(
            (effect) => (effect.disabled = isChecked ? false : true)
          );
  ...
  _onDropPathItem(event, itemData) {
    event.preventDefault();
    if (this.item.type === "profile" || this.item.type === "species")
      return Path.addToItem(this.item, itemData);
    else return false;
  }
  ...
  _onDropCapacityItem(event, itemData) {
    event.preventDefault();
    if (this.item.type === "path" || this.item.type === "species")
      return Capacity.addToItem(this.item, itemData);
    return false;
  }
      ...
      return fromUuid(uuid).then((e) => {
        if (e) return e.sheet.render(true);
          ...
          ui.notifications.error(
            game.i18n.localize("COF.notification.ItemNotFound")
          );
          return false;
        }
      });
